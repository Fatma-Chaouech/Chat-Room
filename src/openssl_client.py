...
class OpenSSLClient:
    def __init__(self, CA, user, certs_path, keys_path, csr_path):
        ...
        self.cert_path = certs_path + '/' + user.name.lower() + '.pem'
        self.key_path = keys_path + '/' + user.name.lower() + '.pem'
    ...
    def __create_csr(self, csr_path):
        ...
        subprocess.run([
            self.CA.openssl_path, 'req', '-new', '-out', csr_path, '-keyout', self.key_path,
